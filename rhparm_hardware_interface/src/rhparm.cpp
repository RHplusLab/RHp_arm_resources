...
const std::string MANUAL_MODE_ENABLE_FILE = "/tmp/rhparm_enable_manual_mode";
...
namespace rhparm
{
	...
	bool rhparm::init()
	{
		...
		std::string dev;
#if defined(RHPARM_USB)
		drvr_ = std::make_unique<rhparm_usb>();
#else
		...
		RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "Joint limits:");
		...
		for (const auto &j : joint_name_map_)
		{
			...
			RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "Joint: %s,  min,max:  %f, %f",
						name.c_str(),
		...
		run_ = true;
		thread_ = std::thread{std::bind(&rhparm::Process, this)};
	...
	void rhparm::setAllJointPositions(const std::vector<double> &commands, const std::vector<std::string> &joints)
	{
		...
		if (std::isfinite(commands[0]) == 1)
		{
			for (uint i = 0; i < commands.size(); i++)
			{
				...
				{
					RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "New pos cmd %*s %s: %.5f",
								i * 8, "",
								name.c_str(),
								commands[i]);
	...
	void rhparm::getAllJointPositions(std::vector<double> &positions, const std::vector<std::string> &joints)
	{
		...
		for (uint i = 0; i < joints.size(); i++)
		{
			...
			RCLCPP_DEBUG(rclcpp::get_logger("RHPArmSystemHardware"), "Get cur pos %*s %s: %.5f",
						 i * 8, "",
						 joints[i].c_str(),
						 positions[i]);
		}
	...
	void rhparm::readJointPositions(PositionMap &pos_map)
	{
		RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "readJointPositions start");
		...
		for (auto const &j : joint_name_map_)
		{
			...
			{
				RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "getJointsPosition error for joint: %d", joint_id);
				continue;
			}
			pos_map[name] = {p, true};
			RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "Read servo %s, pos= %d, %f",
						name.c_str(), p, jointValueToPosition(name, p));
		}
	}

	// Set the specified joint position
	void rhparm::setJointPosition(std::string joint_name, int position, int time)
	{
		RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "Set servo %s, pos= %d, time %d",
					joint_name.c_str(), position, time);

		if (!drvr_->setJointPosition(joint_name_map_[joint_name], position, time))
		{
			RCLCPP_ERROR(rclcpp::get_logger("RHPArmSystemHardware"), "Failed to set joint position for servo %s",
						 joint_name.c_str());
		}
		return;
	}
	...
	bool rhparm::manual_mode_enabled()
	{
	...
	void rhparm::set_manual_mode(bool enable)
	{
		RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "Enable manual mode: %C", enable ? 'Y' : 'N');

		if (!drvr_->setManualModeAll(enable, NUM_JOINTS))
		{
			RCLCPP_ERROR(rclcpp::get_logger("RHPArmSystemHardware"), "Failed to set joint mode enable");
		}
	}
	...
	void rhparm::Process()
	{
		...
		while (run_)
		{
			...
			RCLCPP_DEBUG(rclcpp::get_logger("RHPArmSystemHardware"), "Update");
			...
			if (idle && --ck_for_manual_mode_cnt <= 0)
			{
				...
				if (manual_mode)
				{
					...
					else
					{
						...
						RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "In Manual mode, joint positions:");
						for (auto const &p : last_pos_get_map_)
						{
							RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "  Pos: %d,  Joint: %s", p.second.pos, p.first.c_str());
						}
					}
				}
			...
			if (new_cmd)
			{
				...
				for (auto const &c : cmd)
				{
					if (c.second.changed)
					{
						...
						RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "Update, joint %s, pos= %d, delta= %d",
				...
				if (idle)
				{
					RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "Entering running mode");
					idle = false;
				}
				ck_for_idle_cnt = 0;
			...
			{
				idle = true;
				RCLCPP_INFO(rclcpp::get_logger("RHPArmSystemHardware"), "Entering idle mode");
			}
