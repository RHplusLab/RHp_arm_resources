...
namespace rhparm
{
	...
	{
		RCLCPP_DEBUG(rclcpp::get_logger("RHPArmSystemHardware"), "readJointPosition");
	...
	bool rhparm_serial::setManualModeAll(bool enable, int count)
	{
		...
		for (int i = 0; i < count; i++) {
			...
			if (!bOk) {
				RCLCPP_ERROR(rclcpp::get_logger("RHPArmSystemHardware"), "Failed to set enable mode on servo %d", i + 1);
				bOk = false;
			}
		}
		return bOk;
